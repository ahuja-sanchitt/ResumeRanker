...
## D-001 — Hybrid résumé↔JD scoring (embeddings + LLM-as-judge)
...
- **Status:** Accepted
- **Decision:** Score a résumé against a JD with a blend of embedding cosine similarity and an LLM judgement — `final = 0.6 * embedding_score + 0.4 * llm_fit_score` — and surface all three numbers plus matched/missing skills and feedback.
...
| LLM-as-judge only | Nuanced (seniority, transferable skills, must-haves), explainable (produces matched/missing skills + feedback), rubric-steerable. **But** inconsistent run-to-run, ungrounded numbers (anchors to round numbers, drifts harsh/lenient), gameable by prompt injection (résumé text enters the prompt), pricier/slower, unreliable self-reported confidence. |
