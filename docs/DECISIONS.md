...
## D-001 - Hybrid resume vs JD scoring (embeddings + LLM-as-judge)
...
| Option | Tradeoff |
| --- | --- |
| **Hybrid (both)** (chosen) | Pays for two calls (mitigated by the SHA-256 result cache) and adds combine logic, **but** the two methods fail in opposite directions and cross-check each other. |
